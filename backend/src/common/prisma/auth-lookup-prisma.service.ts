 ...
 * `users` columns - see prisma/manual-sql/001_auth_lookup_role.sql).
 *
 * Why this exists: logging in requires finding a user by email/mobile
 * *before* we know their company_id, but every tenant table is RLS-FORCEd
 * and the normal app role has no way to set app.tenant_id for a company it
 ...
 * beyond the columns AuthService.login() needs, even if application code
 * has a bug.
 *
