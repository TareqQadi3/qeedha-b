...
- **طباعة/PDF فعلي، QR، توقيع رقمي**: لا شيء من هذا في المرحلة 3.
- **الفوترة الإلكترونية ZATCA**: لا UUID فاتورة إلكتروني، لا سلسلة هاش
  (`previous_invoice_hash`)، لا إرسال فعلي لأي جهة — راجع `docs/ZATCA.md`.
  الحقول الحالية (`invoiceNumber`, `taxAmount`, ...) أساس معقول لبناء طبقة
  امتثال لاحقًا، وليست عقدًا نهائيًا لها.
