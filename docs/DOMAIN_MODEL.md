...
- `username` (اختياري، فريد عالميًا مثل `email`/`mobile`) — مُعرِّف دخول
  بديل، انظر "حسابات الفريق" أدناه.

#### حسابات الفريق (Team accounts)
معظم موظفي نقطة البيع/المحاسبة لا يملكون بريدًا إلكترونيًا حقيقيًا
يستخدمونه للعمل. بدل إجبار التاجر على اختلاق بريد لكل موظف،
`IamService.createUser` يقبل `username` كبديل كامل لـ`email`/`mobile` —
يكفي توفير واحد من الثلاثة. تسجيل الدخول (`POST /auth/login`) يقبل
`identifier` كأيٍّ من الثلاثة أيضًا.

**فرق جوهري عن email/mobile في منطق إعادة استخدام الهوية:** عند إنشاء
عضو فريق، إن تطابق `email`/`mobile` مع `User` موجود بالفعل، يُعاد
استخدام تلك الهوية كما هي (نفس المنطق القديم — الشخص نفسه، ربما لديه
عضوية في منشأة أخرى). أما `username` فلا يخضع لهذا المنطق إطلاقًا: هو
مجرد تسمية دخول اختارها التاجر لهذا الموظف تحديدًا، وليس هوية حقيقية
مستقرة قد يحملها الشخص بين منشآت مختلفة. لو عُومِل مثل البريد/الجوال،
لكان تاجر في منشأة B يستطيع (بالخطأ أو عمدًا) اختيار نفس اسم مستخدم
اختاره تاجر في منشأة A، فيُلحَق موظف B الجديد بحساب موظف A الموجود —
تسريب هوية بين منشأتين لا علاقة بينهما. لذلك `IamService.createUser`
يفحص `username` في مسار منفصل تمامًا يرفض بـ`409 Conflict` عند أي
تطابق، ولا يدخل أبدًا في مسار "إعادة استخدام هوية موجودة".

Cashier/Accountant/Inventory Manager هي أدوار نظامية جاهزة (`Role`
بـ`companyId = NULL`) بصلاحيات مختلفة مسبقًا لكل منها — إنشاء حساب
فريق بدور مختلف يكفي وحده لمنح صلاحيات مختلفة، بلا حاجة لأي إعداد
إضافي من التاجر.
...
(عام، بلا مصادقة، بحد أقصى 5 طلبات/دقيقة لكل IP - يُعيد فروعًا نشطة فقط،
**بلا اسم الشركة**: بما أن `subscriptionNumber` رقم تسلسلي بسيط يبدأ من
10001، إعادة اسم الشركة هنا كانت ستجعل هذا المسار أداة لحصر كل الشركات
المسجَّلة بالمنصة بمجرد تجربة الأرقام تباعًا - راجع `docs/SECURITY.md`
"تعداد رقم الاشتراك"). بعد
...
## Platform admin — لوحة تحكم SaaS

**ليست Milestone جديدة — ميزة مطلوبة صراحةً.** فاعل جديد كليًا، منفصل
تمامًا عن كل ما سبق في هذا المستند: `PlatformAdmin` (نظام Anthropic/قيّدها
الداخلي، وليس تاجرًا ولا موظف تاجر). القرار المعماري المحوري: **لا
يُبنى فوق نموذج `User`/`Membership` إطلاقًا** - يُنمذَج ككيان جديد كليًا
بسبب فرق جوهري في مستوى الثقة.

### لماذا نموذج هوية مستقل تمامًا، لا مجرد دور إضافي فوق User/Membership

كل ما هو موجود في هذا المستند مبني على افتراض واحد ثابت: **كل طلب API
يعمل ضمن سياق منشأة واحدة بالضبط** (`Current Tenant Context` أعلاه) -
`Membership` تربط شخصًا بمنشأة واحدة، والـaccess token يحمل `companyId`
واحدًا، و`RLS` يفرض هذا على مستوى قاعدة البيانات ذاتها. مدير المنصة
يحتاج بالضبط عكس هذا: رؤية **كل** المنشآت معًا، لا منشأة واحدة. لو
أُضيف كـ"دور نظامي" فوق `Role`/`MembershipRole` الموجودين، لكان كل طلب
له لا يزال يحتاج `companyId` واحدًا ليمر عبر `RLS` - تناقض بنيوي، ليس
تفصيل تنفيذ يمكن التحايل عليه. لذلك:

- **`PlatformAdmin`**: جدول جديد كليًا (`id, fullName, email, passwordHash,
  status, createdAt, updatedAt`) - عالمي مثل `User`، لا `companyId`، لا
  RLS إطلاقًا. لا `Membership` له بأي منشأة.
- **جلسة/JWT منفصلان تمامًا**: `JWT_PLATFORM_ADMIN_SECRET` (سرّ مستقل عن
  `JWT_ACCESS_SECRET`)، حمولة `{sub, scope: 'platform_admin'}` بدل
  `{sub, companyId, membershipId}`. لا refresh token (توكن واحد أطول
  أجلًا - 8 ساعات افتراضيًا) - هذا استخدام تشغيلي داخلي منخفض التكرار،
  ليس جلسة عميل تفاعلية طويلة الأمد.
  `PlatformAdminAuthGuard` (`src/modules/platform-admin/`) يتحقق من هذا
  التوكن يدويًا، منفصل تمامًا عن `JwtAuthGuard`/`MembershipGuard`
  العاديين - المسارات كلها `@Public()` + `@UseGuards(PlatformAdminAuthGuard)`
  صراحةً (نفس نمط `QeedhaIntegrationAuthGuard` أعلاه، لنفس السبب: فاعل
  بمستوى ثقة مختلف جذريًا يجب ألا يشارك أي جزء من سلسلة حراسة تفترض جلسة
  Membership).
- **دور Postgres ثالث `qeedha_platform_admin`** (`BYPASSRLS`، `SELECT`
  محدود على أعمدة `companies` فقط - `prisma/manual-sql/004_platform_admin_role.sql`):
  يحل مشكلة "قبل معرفة أي tenant context" بنفس فلسفة `qeedha_auth_lookup`
  بالضبط، لكن **دور منفصل عمدًا** - لم يُوسَّع `qeedha_auth_lookup`
  الموجود لهذا الغرض، لأنه موثَّق ومُستخدَم حصرًا لمسار bootstrap تسجيل
  الدخول البشري (`AuthLookupService`)؛ توسيعه هنا كان سيُنمّي صلاحياته
  بصمت لميزة غير متعلقة. كل ميزة جديدة تحتاج تجاوز RLS تحصل على دورها
  الضيق الخاص بها.
- **إنشاء منشأة تاجر**: `PlatformAdminService.createCompany` يستدعي
  `AuthService.createCompanyWithOwner` مباشرةً - **نفس** منطق
  `POST /auth/register-company` بالضبط (نفس الفرع/المستودع الافتراضي،
  نفس دليل الحسابات، نفس الاشتراك التجريبي)، مُستخرَج إلى دالة واحدة
  مشتركة بدل تكرار المنطق. **لا يحتاج أي تجاوز RLS**: بما أن معرّف
  المنشأة الجديدة يُولَّد سلفًا ثم `withTenant(companyId, ...)` تُفتح
  فورًا لهذا المعرّف الجديد بالذات، فكل الإدراجات تمر عبر `RLS` العادي
  بلا مشكلة - "قبل وجود صفوف لهذه المنشأة" وليس "قبل معرفة أي منشأة"
  (نفس الحيلة المستخدمة في `registerCompany` نفسها). لا tokens تُصدَر
  للمدير بصفته المالك - التاجر نفسه يسجّل دخوله لاحقًا ببيانات الاعتماد
  التي أدخلها المدير عنه.
- **لا تسجيل ذاتي لمديري المنصة إطلاقًا** - يُبذَر مدير واحد فقط عبر
  `prisma/seed.ts` عند تعيين `PLATFORM_ADMIN_SEED_EMAIL`/
  `PLATFORM_ADMIN_SEED_PASSWORD` صراحةً (Idempotent، لا `POST` عام لإنشاء
  مدير منصة).

### الواجهة الأمامية

مسار منفصل تمامًا خارج `Layout`/`AuthProvider` التاجر (`/admin/login`،
`/admin`) بمزوّد حالة مصادقة مستقل (`AdminAuthProvider`) وعميل API
منفصل (`admin/adminApi.ts` - مخزَّن محليًا بمفتاح مختلف تمامًا عن توكن
جلسة التاجر، لا منطق تجديد توكن لأنه لا يوجد refresh token هنا أصلًا).
لوحة واحدة: قائمة كل المنشآت + نموذج إنشاء منشأة جديدة يعرض بيانات دخول
المالك بعد الإنشاء مباشرةً لتسليمها للتاجر.

راجع `docs/API.md` "لوحة تحكم SaaS" لعقد `/platform-admin/*` الكامل،
و`backend/test/platform-admin.e2e-spec.ts` للسلوك المُتحقَّق منه فعليًا
(تسجيل الدخول، رفض توكن جلسة تاجر عادية في مسارات المنصة والعكس، إنشاء
منشأة وظهورها في القائمة، وتسجيل دخول التاجر نفسه فورًا بالبيانات التي
أدخلها المدير).
