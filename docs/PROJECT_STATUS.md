...
**آخر تحديث**: 2026-08-25 — **Phase 13 (باقات مُخصَّصة لكل دور + تخصيص لكل
منشأة من لوحة التحكم) منفَّذة بالكامل**: أربع باقات جديدة
(`basic`/`standard`/`premium`/`enterprise` — "الأولى"/"الثانية"/"الثالثة"/
"المؤسسات") بحدود مضبوطة **لكل دور نظام** بدل إجمالي مستخدمين واحد: فرع 1/
نقاط بيع 2/محاسب 1/مدير 0/مخزن 1 للأولى، وهكذا حتى الثالثة (فرع 3/نقاط
بيع 6/محاسب 1/مدير 1/مخزن 2)، والمؤسسات بلا حدود افتراضية. `starter`/
`professional` القديمتان **لم تُمسّا إطلاقًا** — أُضيفت الأربع الجديدة
بجانبهما فقط، فلا تراجع في أي من الـ270 اختبار e2e الموجود سلفًا الذي
يعتمد على حدودهما الواسعة. `Plan` يكتسب `maxCashiers`/`maxAccountants`/
`maxManagers`/`maxWarehouses` (نفس أدوار `Cashier`/`Accountant`/`Manager`
النظامية الموجودة سلفًا بالاسم، لا أدوار جديدة)، و`Subscription` يكتسب
ستة أعمدة تجاوز اختيارية بالإضافة إلى `productsOverride` — "أخصّص أي باقة
من لوحة التحكم": مدير المنصة يرفع/يخفض أي حد لمنشأة واحدة بعينها (`null`
= اتّبع حد الباقة، أي قيمة أخرى — بما فيها صفر — تَغلب)، أو يفعّل خدمة
قيّدها (التقسيط) لمنشأة واحدة دون تغيير باقتها المشتركة مع عملاء آخرين،
عبر `POST /platform-admin/companies/:id/subscription/overrides` الجديد
(مع `GET .../subscription` لعرض الاستخدام الفعلي مقابل الحد الفعّال قبل
التعديل). توسيع نطاق فرع لدور موجود بالفعل (لا عدد جديد) لا يُصطدم بالحد
أبدًا. لوحة التحكم (Control Center) ولوحة التاجر كلتاهما تعرضان الحدود/
الاستخدام الجديدة. 13 اختبار e2e جديد (`phase13.e2e-spec.ts`) + اختبار
Vitest جديد لصفحة اشتراك التاجر، والمجموعة الكاملة (283 اختبارًا) تعمل
بلا أي تراجع. راجع `docs/DOMAIN_MODEL.md` §"الباقات (Phase 13)" للتفصيل
الكامل. لم تُمَس أي منطق Wallet/Ledger/دفع فعلي - لا بوابة دفع حقيقية
موجودة أصلًا، فالتخصيص يُضبَط يدويًا من لوحة التحكم كما هو مذكور في
Phase 9.

**آخر تحديث سابق**: 2026-08-22 — **Phase 12 (رقم الاشتراك + تسجيل دخول الموظف)
منفَّذة بالكامل**: كل منشأة جديدة تحصل على `subscriptionNumber` تسلسلي
فريد (`Company.subscriptionNumber`، يبدأ من 10001) - مُعرِّف يمليه التاجر
لفظيًا، يُعاد في استجابة التسجيل و`/auth/me`. دخول التاجر (`/auth/login`)
يقبل الآن 3 مُعرِّفات: بريد/جوال/رقم اشتراك (الأخير يُحل عبر Membership
الـOwner الوحيدة للمنشأة). دخول جديد كليًا لحسابات الفريق
(`POST /auth/employee-login`): رقم اشتراك + فرع (من قائمة
`GET /auth/companies/:subscriptionNumber/branches`) + اسم مستخدم + كلمة
مرور، مع التحقق من أن دور الموظف يشمل الفرع المختار (403 إن لم يكن).
الإصلاح الجوهري المصاحب: `User.username` كان فريدًا **عالميًا** (منشأتان
مختلفتان لا تستطيعان تسمية موظف بنفس الاسم) - أصبح فريدًا **لكل منشأة**
(`User.homeCompanyId` + `@@unique([homeCompanyId, username])`)، فالتصادم
بين منشآت غير مرتبطة اختفى تمامًا. `/auth/login` لم يعد يطابق `username`
(الآن حصريًا عبر `employee-login`) تفاديًا لمطابقة حساب من منشأة خاطئة.
270 اختبار e2e يعمل (بما فيها اختبارات جديدة لعزل الفروع/المنشآت)، تحقّق
فعلي في متصفح حقيقي لتدفّق دخول الموظف كاملًا. راجع `docs/DOMAIN_MODEL.md`
§"رقم الاشتراك ودخول الموظف (Phase 12)" للتفصيل الكامل. لم تُمَس أي منطق
Wallet/Ledger/دفع - هذا تعديل هوية/دخول بحت.

**آخر تحديث سابق**: 2026-08-20 — **Phase 11 (Qeedha Integration API Foundation)
...
طبقة تجريد مزوّد دفع اشتراكات (`SubscriptionPaymentProvider`، لا مزوّد
حقيقي بعد — `charge()` يرفض دائمًا بدل تزييف نجاح)، مزوّد بريد SMTP حقيقي
عبر `nodemailer` إلى جانب مزوّد الطرفية الموجود، حماية تزامن على مستوى
قاعدة البيانات لبريد/جوال التسجيل (فهارس فريدة جزئية)، ولوحة تحكم ذاتية
جديدة للمسوّقين بالعمولة (تسجيل دخول مستقل + عرض إحالاتهم وعمولاتهم
فقط). راجع `docs/WEBSITE.md` القسم 20 للتقرير الكامل. لا مزوّد دفع حقيقي
ولا تكامل Qeedha فعلي بعد — تلك مراحل لاحقة.

**آخر تحديث سابق**: 2026-08-19 — **Website Phase منفَّذة بالكامل**: الموقع
التسويقي العام (`/site/*`)، رحلة تسجيل التاجر الكاملة (دولة/باقة/إحالة/
تأكيد بريد)، نظام التسويق بالعمولة، طلبات الانضمام، ومركز التحكم موسَّع
بخمسة أدوار موظفين. راجع `docs/WEBSITE.md` للتقرير الكامل و
`docs/CHANGELOG.md` للملخّص. لم تُعدَّل أي وظيفة محاسبية/POS موجودة.
...
# Milestone 10 — الإصدار الإنتاجي النهائي (Final Production Release)

**تاريخ**: 2026-08-17

## ملخص: ما هذا الـMilestone وما ليس

**هذا آخر Milestone في خارطة الطريق المتفق عليها.** لا Milestone 11 ولا
أي تسليم إضافي مخطَّط. الهدف: تدقيق إنتاجي شامل على القاعدة الموجودة
فعليًا (Milestones 1-9)، إصلاح أي عيب إنتاجي حقيقي يُكتشَف ويكون آمنًا
ومعقولًا إصلاحه الآن، توثيق نهائي دقيق يميّز بوضوح بين "منفَّذ" و"غير
منفَّذ" و"محجوب باعتماد خارجي" و"قيد معروف"، ثم تحقق كامل وcommit واحد.
**ليس** ميزة كبيرة جديدة، **ليس** إعادة بناء أي نظام يعمل فعليًا، **ليس**
بدء أي عمل من: Control Center، Website، Affiliate، تطبيق موبايل أصلي،
تحليلات متقدمة، ZATCA Phase 2، مزوّدي تمويل خارجيين — كل هذه خارج خارطة
الطريق نهائيًا.

## عيوب إنتاجية حقيقية اكتُشفت وأُصلِحت (وليست ميزات جديدة)

اكتشاف الأربعة التالية تطلَّب محاولة تطبيق فعلية للسكربتات/التهيئة من
الصفر في هذه الجلسة — لم تكن مرئية من قراءة الكود وحدها:

1. **ثغرة تهيئة إنتاجية**: `NODE_ENV=production` كان يقبل بصمت أسرار
   JWT/تشفير لا تزال تحمل القيم الحرفية الافتراضية من `.env.example`
   (`change-me-...`)، أو قصيرة جدًا، أو مُكرَّرة بين أكثر من غرض. أُضيف
   فحص فاشل-إغلاقًا جديد (`assertSecretsProductionSafe`) يرفض البدء في
   الإنتاج بأي من هذه الحالات — نفس نمط `assertCorsConfiguredForProduction`
   الموجود. **5 اختبارات وحدة جديدة** تغطيه مباشرة.
2. **سكربت `manual-sql/001_auth_lookup_role.sql` كان معطوبًا دائمًا في
   جزء منه، منذ Auth/IAM identity refactor**: `GRANT SELECT` على عمود
   `users.company_id` المحذوف يفشل حتمًا على أي تطبيق حقيقي (ويُلغى فورًا
   بواسطة 002 على أي حال) — أُزيل. `GRANT CONNECT ON DATABASE
   qeedha_accounting` كان يحمل اسم قاعدة بيانات ثابتًا خاطئًا لأي بيئة
   غير التطوير المحلي (يفشل صامتًا على CI/الاختبار/أي اسم إنتاجي آخر) —
   أُصلِح ليستخدم `current_database()` ديناميكيًا.
3. **CI كان سيفشل بصمت في مصادقة تكامل قيّدها**: `.github/workflows/ci.yml`
   طبَّق سكربتَي 001/002 فقط، بلا 003 (منح Milestone 9 على
   `integration_connections`) — أي تشغيل CI حقيقي منذ Milestone 9 كان
   سيُفشِل كل اختبارات `QeedhaIntegrationAuthGuard` بـ401. أُضيف 003 في
   وظيفتَي `backend` و`e2e`.
4. **توثيق نشر ناقص**: خطوات `docker-compose`/إعادة تصفير Demo في
   `docs/DEPLOYMENT.md` كانت تطبّق 001/002 فقط أيضًا (نفس ثغرة CI، لكن
   في وثيقة النشر اليدوي) — أُضيف 003 في كل مكان مُطابق، بالإضافة لتحذير
   جديد حول اختلاف مضيف قاعدة البيانات (`localhost` مقابل اسم خدمة
   `postgres`) بين التشغيل المحلي والتشغيل عبر compose، وهو خطأ شائع لم
   يكن موثَّقًا من قبل.

كل الإصلاحات الأربعة أُعيد اختبارها فعليًا بعد التطبيق (إعادة تشغيل
سكربتات manual-sql من الصفر ضد قاعدتي dev وtest، التحقق المباشر من
المنح عبر `information_schema.role_column_grants`/`pg_database.datacl`،
ثم مجموعة الاختبارات الكاملة).

## توثيق مُصحَّح (كان يصف مُنفَّذًا كأنه مفقود، أو العكس)

- `docs/ARCHITECTURE.md`: قسم "Monorepo Layout" كان لا يزال لقطة Phase 2
  (يصف `sales`/`purchasing`/`accounting`/`import`/`zatca` كـ"تُضاف
  بالمراحل القادمة")؛ قسم "القرارات المفتوحة" كان يصف أسئلة حُسمت منذ
  عدة مراحل كأنها لا تزال مفتوحة. كلاهما أُعيد كتابته ليعكس الحالة
  الفعلية النهائية.
- `docs/ROADMAP.md`: الجدول بالكامل كان لا يزال يصف كل شيء بعد Phase 1
  بـ"لم تبدأ" — أُعيد كتابته بالكامل كسجل زمني حقيقي لكل ما اكتمل فعليًا،
  مع إغلاق صريح لخارطة الطريق عند Milestone 10.
- `docs/INVOICES.md`: كان يصف رمز QR كـ"لا شيء من هذا في المرحلة 3" رغم
  أن ZATCA Phase 1 (QR) اكتمل فعليًا منذ Milestone 4 — صُحِّح.
...
| RLS | PASS |
| RBAC | PASS |
| Authentication (تدوير refresh token + كشف إعادة الاستخدام) | PASS |
| Subscription enforcement | PASS |
| Qeedha Integration | PASS |
| Accounting integrity | PASS |
| Inventory integrity | PASS |
| Security (أسرار/CORS/Helmet/Logging/راجع القسم أعلاه) | PASS |
| CORS | PASS |
| Rate limiting | PASS |
| Logging (بلا أي سر مُسجَّل) | PASS |
| Health (`/health` يُعيد 503 عند فشل القاعدة) | PASS |
| Database migrations | PASS (لا drift على dev/test) |
| Backend tests | **242/242** e2e + **9/9** unit = **251/251** |
| Frontend tests | **53/53** Vitest |
| Playwright | **4/4** |
| Build (backend + frontend) | PASS |
| Lint (backend + frontend) | PASS |
| Typecheck (backend + frontend) | PASS |
| Docker | BLOCKED BY ENVIRONMENT (محاولة `dockerd` فعلية فشلت هذه الجلسة، `docker compose config` نجح ثابتًا) |
| CI | SYNTAX/STATIC VERIFIED، REAL RUNNER UNAVAILABLE (وأُصلِح عيب حقيقي فيه هذه الجلسة) |
| Secret scan | PASS |
| Documentation | PASS (مُحدَّثة لتعكس الحالة النهائية الفعلية) |

## القيود المعروفة (حقيقية، غير حاجبة لهذا الإصدار)

- **Docker غير مُختبَر فعليًا بـ`build`/`up` حقيقيَّين** — لا daemon متاح
  في أي بيئة تطوير توفّرت حتى الآن، بما فيها محاولة صريحة في هذه الجلسة.
- **CI غير مُشغَّل فعليًا على GitHub Actions runner حقيقي** — تحقق
  ثابت (syntax + `docker compose config`-مثيل التحقق) فقط.
- **لا نشر Demo/Staging فعلي** — لا استضافة أو اعتمادات متاحة.
- **localStorage لتخزين tokens في الواجهة الأمامية** (بدل httpOnly
  cookies) — قرار معماري قائم منذ البداية، موثَّق كقيد أمني معروف بدل
  إعادة بناء معمارية المصادقة في هذا الإصدار.
- **اتجاه Outbound لتكامل قيّدها غير مُنفَّذ** — BLOCKED BY EXTERNAL
  DEPENDENCY (عقد API خارجي من قيّدها غير متاح).
- **ZATCA Phase 2 غير مُنفَّذ** — BLOCKED BY EXTERNAL DEPENDENCY
  (اعتمادات CSID حقيقية وقرار عمل/قانوني حول الموجة غير متاحين).
- **لا تكامل تحصيل دفع خارجي حقيقي للاشتراكات** (Stripe/Moyasar/...) —
  قرار نطاق صريح منذ Milestone 8.
- **Offline POS غير مُنفَّذ** — قرار نطاق موثَّق منذ Phase 3، لن يُبنى
  ضمن خارطة الطريق هذه.

هذه القيود **حقيقية وموثَّقة بصدق**، وليست عيوبًا حاجبة لإصدار المنتج
ضمن نطاقه المتفق عليه — كل واحدة منها إما اعتماد خارجي غير متاح، أو قرار
نطاق مُتعمَّد ومُوثَّق منذ مرحلة سابقة.

## نطاق مستقبلي/خارجي (خارج خارطة الطريق نهائيًا)

Control Center · Website · Affiliate · اتجاه Outbound الفعلي لتكامل
قيّدها · ZATCA Phase 2 · مزوّدو تمويل خارجيون · تطبيق موبايل أصلي ·
عملات متعددة · تحليلات متقدمة · FIFO/تكلفة مخزون متقدمة · Background
Queues · httpOnly cookie-based auth. أي عمل على أي بند من هذه يتطلب قرار
نطاق جديد صريح من خارج خارطة الطريق المتفق عليها — لا يبدأ تلقائيًا بعد
هذا الـMilestone.
...
## Push
NOT PUSHED — ولن يُدفَع تحت أي ظرف.
...
**Qeedha B يُعتبَر مكتملًا وفق خارطة الطريق المتفق عليها عند إتمام هذا
الـMilestone.** لا Milestone إضافي يبدأ تلقائيًا بعده.

---

# تحقق Go-Live الإنتاجي (بعد Milestone 10 — ليس Milestone جديدًا)

**تاريخ**: 2026-08-18

هذه ليست ميزة أو Milestone جديد — تحقق تشغيلي إضافي فقط على القاعدة
المكتملة فعليًا (Milestone 7-10)، يهدف لرفع الثقة من "مُختبَر بوحدات/e2e"
إلى "مُشغَّل فعليًا في تهيئة إنتاجية حقيقية محليًا". راجع
`docs/DEPLOYMENT.md` §"تحقق Go-Live فعلي" للتفصيل الكامل خطوة بخطوة.

## ما تحقَّق فعليًا وحيًا (وليس نظريًا)

- تدفق `prisma migrate deploy` الحتمي يعمل من الصفر على قاعدة بيانات
  جديدة تمامًا (`qeedha_accounting_prodsim`، أُنشئت وحُذفت لهذا التحقق
  فقط — dev/test لم تُلمَسا).
- سكربتات `manual-sql` الثلاثة (001+002+003) تُطبَّق بنجاح كامل على
  قاعدة جديدة، والمنح تحقَّقت بالاستعلام المباشر.
- `NODE_ENV=production` **يرفض فعليًا** البدء بسر placeholder (تحقَّق
  حي، ليس اختبار وحدة فقط) و**ينجح فعليًا** بأسرار حقيقية قوية.
- CORS Allowlist وHelmet Headers تحقَّقا حيًا عبر طلبات HTTP فعلية ضد
  الخادم الحي.
- بناء الواجهة الأمامية للإنتاج (`vite build`) صُدِّق ثم قُدِّم، ومجموعة
  Playwright الكاملة (4/4) نُفِّذت ضده فعليًا (لا خوادم تطوير).
- مصادقة تكامل قيّدها الخارجية (`QeedhaIntegrationAuthGuard`) تحقَّقت
  حيًا بـ`curl` كامل (ربط → سر خاطئ يُرفَض 401 → سر صحيح ينجح 201) — أول
  مرة يُختبَر فيها هذا المسار خارج مجموعة الاختبارات الآلية.
- لا سر ظهر في أي سطر Log فعلي فُحص مباشرة.

## Production Checklist

- [x] Production environment configured (فحص فاشل-إغلاقًا مُتحقَّق حيًا)
- [x] Strong secrets configured (تحقَّق: placeholder يُرفَض، سر قوي يُقبَل)
- [x] Database provisioned (قاعدة جديدة، `migrate deploy` من الصفر)
- [x] Prisma migrations deployed (17/17، بلا drift)
- [x] qeedha_auth_lookup grants applied (001+002+003، تحقَّق مباشر)
- [x] Backend runs in production mode (`node dist/main`، مُتحقَّق حيًا)
- [x] Frontend production build verified (`vite build` + Playwright حي ضده)
- [ ] HTTPS configured — **يتطلب إجراء المُشغِّل** (راجع أدناه)
- [ ] Domain configured — **يتطلب إجراء المُشغِّل**
...
- [x] Tenant isolation verified (Playwright + e2e مباشر)
- [x] Production logs verified (بلا أي سر)
- [ ] Backup strategy — **إجراء تشغيلي للمُشغِّل** (راجع أدناه، لا نظام
      نسخ احتياطي آلي مبني في هذا المستودع، ولا يُفترَض بناؤه)
- [x] Rollback procedure documented (راجع أدناه)

## متطلبات خارجية تتطلب إجراء المُشغِّل (Operator Action Required)

هذه بنود **لا يمكن لأي جلسة تطوير في هذه البيئة إكمالها** — تتطلب بنية
تحتية/اعتمادات حقيقية خارج هذا المستودع:

- **دومين إنتاجي حقيقي** وشهادة **TLS/HTTPS** (Let's Encrypt أو مزوّد
  آخر) — لا قيمة وهمية تُخترَع هنا.
- **Reverse proxy حقيقي** (nginx/Traefik/Caddy أمام الحاويتين) لإنهاء
  TLS وتوجيه `/api` للـbackend والباقي للواجهة الأمامية الثابتة — ملف
  `frontend/nginx.conf` الحالي يخدم الواجهة الأمامية فقط داخل حاويتها،
  وليس Reverse proxy للنظام كاملًا.
- **قيمة `CORS_ALLOWED_ORIGINS` الإنتاجية الحقيقية** = دومين الواجهة
  الأمامية المنشورة فعليًا (HTTPS، ليس `localhost`).
...
## استراتيجية النسخ الاحتياطي/الاستعادة والـRollback (إجراء تشغيلي، لا نظام مبني)

لا نظام نسخ احتياطي آلي في هذا المستودع، ولم يُبنَ واحد في هذا التحقق
(خارج النطاق صراحةً). الإجراءات الموصى بها للمُشغِّل:

- **نسخ احتياطي لقاعدة البيانات**: `pg_dump`/`pg_restore` (موثَّق فعليًا
  في `docs/DEPLOYMENT.md` §6) — يحتاج جدولة خارجية حقيقية (cron/خدمة
  استضافة) غير موجودة في هذا المستودع.
- **Rollback للتطبيق**: نشر صورة Docker السابقة (tag سابق) — يتطلب سجل
  صور (registry) حقيقي غير متاح هنا.
- **Rollback للـmigrations**: **لا** `prisma migrate reset`/`down` تلقائي
  في الإنتاج أبدًا (قرار مُتعمَّد) — أي عكس Schema يحتاج migration
  عكسية صريحة تُكتَب وتُختبَر يدويًا، تمامًا كأي migration جديدة أخرى.
- **استعادة متغيرات البيئة**: تُدار عبر أداة أسرار المُشغِّل الخاصة به
  (Vault/متغيرات بيئة منصة الاستضافة) — لا آلية مُخزَّنة في هذا
  المستودع (ولا يجب أن تكون).
- **استعادة التخزين**: نسخ احتياطي لحجم `STORAGE_LOCAL_DIR` بشكل منفصل
  عن قاعدة البيانات، أو الانتقال لمزوّد S3-compatible له نسخه الاحتياطي
  الخاص (راجع `docs/IMPORT_EXCEL.md` §1).

## الخلاصة

Qeedha B **جاهز تقنيًا للنشر** ضمن حدود ما يمكن التحقق منه في بيئة
تطوير بلا Docker daemon ولا بنية تحتية إنتاجية حقيقية. كل ما تبقّى
(HTTPS/دومين/Docker/استضافة/نسخ احتياطي مُجدوَل) **يتطلب إجراءً حقيقيًا
من المُشغِّل خارج نطاق أي جلسة تطوير**، وليس عملًا برمجيًا إضافيًا في
هذا المستودع.
