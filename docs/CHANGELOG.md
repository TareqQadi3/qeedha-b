...
(`PlatformAdmin`) يستطيع رؤية كل منشآت المنصة وإنشاء منشآت تجار جديدة
بالنيابة عنهم - راجع docs/DOMAIN_MODEL.md "Platform admin" للتصميم
الكامل والمبرر المعماري.

### الخلفية (Backend)
- `PlatformAdmin` جدول جديد كليًا (عالمي، لا RLS، لا `Membership`) -
  منفصل تمامًا عن `User`/`Membership`، لأن "يرى كل المنشآت" و"مُقيَّد
  بمنشأة واحدة" مستويا ثقة مختلفان جذريًا لا يجوز أن يشتركا في نفس
  جلسة/JWT.
- جلسة/JWT مستقلان تمامًا (`JWT_PLATFORM_ADMIN_SECRET` منفصل، حمولة
  `{sub, scope: 'platform_admin'}`، توكن واحد 8 ساعات بلا refresh) -
  `PlatformAdminAuthGuard` جديد، مُطبَّق فقط عبر `@UseGuards()` +
  `@Public()` (نفس نمط `QeedhaIntegrationAuthGuard`)، لا يشارك أي جزء من
  سلسلة حراسة التاجر العادية.
- دور Postgres ثالث `qeedha_platform_admin` (`BYPASSRLS` + `SELECT`
  محدود على `companies` فقط - `prisma/manual-sql/004_platform_admin_role.sql`)،
  منفصل عمدًا عن `qeedha_auth_lookup` الموجود لتفادي توسيع صلاحياته
  لغرض غير متعلق.
- `POST /platform-admin/companies` يستدعي منطق إنشاء المنشأة+المالك
  نفسه المُستخدَم في التسجيل الذاتي (`AuthService.createCompanyWithOwner`،
  استُخرِج من `registerCompany` القائم بلا تغيير سلوكي) - لا يُصدر
  tokens للمدير، التاجر يسجّل دخوله بنفسه لاحقًا.
- 5 اختبارات e2e جديدة (`test/platform-admin.e2e-spec.ts`): تسجيل
  الدخول، رفض بيانات خاطئة، رفض توكن جلسة تاجر في مسارات المنصة والعكس،
  إنشاء منشأة وظهورها في القائمة وتسجيل دخول التاجر فورًا بالبيانات نفسها،
  رفض أي طلب بلا توكن.

### الواجهة (Frontend)
- مسار منفصل تمامًا خارج `Layout`/جلسة التاجر: `/admin/login` و`/admin`،
  بمزوّد حالة (`AdminAuthProvider`) وعميل API (`admin/adminApi.ts`)
  مستقلَّين كليًا عن جلسة/عميل التاجر.
- لوحة واحدة: قائمة كل المنشآت + نموذج إنشاء منشأة جديدة يعرض بيانات
  دخول المالك مباشرة بعد الإنشاء لتسليمها للتاجر.
- namespace ترجمة جديد `admin` (عربي/إنجليزي)، اختبارات Vitest لصفحتي
  الدخول واللوحة.

## [حسابات الفريق: اسم مستخدم/كلمة مرور بلا بريد إلكتروني] - 2026-08-19

**ليست Milestone جديدة — ميزة مطلوبة صراحةً.** التاجر كان يحتاج بريدًا
إلكترونيًا أو رقم جوال حقيقي لإنشاء أي حساب فريق (نقطة بيع/محاسب/...)،
وهو ما لا يملكه عادةً كل موظف. أضيف `username` كمُعرِّف بديل اختياري -
راجع docs/DOMAIN_MODEL.md "حسابات الفريق (Team accounts)".

### الخلفية (Backend)
- `prisma/schema.prisma` + migration `20260819040000_add_user_username`:
  عمود `User.username` (اختياري، فريد عالميًا `@unique`).
- `POST /iam/users` (`CreateUserDto`): حقل `username` جديد (3-32 حرفًا/رقمًا
  إنجليزيًا، بلا مسافات)، يُقبَل بديلًا عن `email`/`mobile` - يكفي أحد
  الثلاثة. `IamService.createUser`: خلافًا للبريد/الجوال، تطابق اسم
  المستخدم **لا يُعيد استخدام هوية مستخدم موجودة أبدًا** (فحص مستقل يرفض
  بـ 409 إن كان مُستخدَمًا) - لأن اسم المستخدم مجرد تسمية دخول اختارها
  التاجر لهذا الموظف، وليس هوية حقيقية مستقرة قد يحملها الشخص بين
  منشآت مختلفة كالبريد/الجوال؛ دمجه في نفس منطق إعادة الاستخدام كان
  سيُلحِق موظفًا جديدًا في منشأة بحساب شخص آخر تمامًا في منشأة مختلفة
  فقط لأنهما اختارا نفس اسم المستخدم.
- `POST /auth/login`: يقبل `identifier` = بريد إلكتروني **أو** رقم جوال
  **أو** اسم مستخدم.
- دور Cashier الافتراضي: أُضيفت له `tenancy.warehouses.view` (كانت
  مفقودة، فكانت شاشة نقطة البيع - التي يعمل عليها الكاشير حصرًا - تفشل
  في التحميل بالكامل قبل هذا الإصلاح).
- 4 اختبارات e2e جديدة (`test/team-accounts.e2e-spec.ts`): إنشاء حساب
  نقطة بيع/محاسب باسم مستخدم فقط، فرق الصلاحيات بين الدورين، رفض اسم
  مستخدم مكرر بين منشأتين مختلفتين (409)، رفض إنشاء حساب بلا أي مُعرِّف.

### الواجهة (Frontend)
- صفحة جديدة `/team` (`TeamPage.tsx`): قائمة أعضاء الفريق، نافذة إنشاء
  عضو جديد (اسم الموظف/اسم المستخدم/كلمة المرور/الدور/الفرع الاختياري -
  بلا حقل بريد إلكتروني إطلاقًا)، إضافة/إزالة دور لعضو موجود. دور
  "Integration" (فاعل نظامي غير بشري) مستبعد من قائمة الأدوار القابلة
  للإسناد. محمية بصلاحية `iam.users.view`/`iam.users.manage`.
- عنصر تنقّل "الفريق" جديد في الشريط الجانبي.
- حقل تسجيل الدخول يوضّح الآن قبول اسم المستخدم أيضًا ("البريد
  الإلكتروني أو رقم الجوال أو اسم المستخدم").
- namespace ترجمة جديد `team` (عربي/إنجليزي)، اختبارات Vitest للصفحة،
  وامتداد لمسار Playwright الكامل (`golden-path.spec.ts`): التاجر ينشئ
  حساب كاشير، يسجّل الكاشير الدخول باسم المستخدم، ويُتحقَّق فعليًا أنه
  محجوب عن `/team` و`/accounting` بينما يعمل على `/pos` بلا مشاكل.

## [Frontend Bilingual: عربي/إنجليزي عبر i18next] - 2026-08-19

**ليست Milestone جديدة — ميزة واجهة مطلوبة صراحةً.** أضيف دعم لغتين
كاملتين (عربي/إنجليزي) للواجهة الأمامية عبر `i18next`/`react-i18next`.
العربية تبقى الافتراضية دائمًا؛ الإنجليزية اختيار صريح عبر مبدّل اللغة،
غير مُكتشَف تلقائيًا من `navigator.language` (لضمان بقاء كل الاختبارات
الحالية التي تستعلم بنص عربي دقيق صحيحة دون أي تعديل).

### البنية
- `src/i18n.ts`: تهيئة i18next، 19 namespace (صفحة/قسم لكل namespace)،
  كشف اللغة الأولية من `localStorage` فقط (افتراضي `ar`)، ضبط
  `document.documentElement.lang/dir` تلقائيًا عند أي تغيير لغة.
- `src/components/LanguageSwitcher.tsx`: مبدّل بسيط (عربي/إنجليزي)، مُدرَج
  في الشريط الجانبي (`Layout.tsx`) وصفحتَي الدخول/التسجيل (خارج
  `Layout`).
- `src/locales/{ar,en}/*.json`: 19×2 ملف ترجمة (namespace لكل صفحة تقريبًا:
  common، nav، auth، dashboard، pos، sales، products، catalog، inventory،
  party, purchases، imports، expenses، accounting، reports،
  receivablesPayables، subscription، integration، onboarding). القيم
  العربية منسوخة حرفيًا من النص الأصلي (لا تغيير في الصياغة) - كل اختبار
  Vitest/Playwright يستعلم بنص عربي دقيق ما زال يعمل بلا أي تعديل على
  ملفات الاختبار نفسها. القيم الإنجليزية ترجمة مهنية بمصطلحات محاسبية
  معتادة، وليست ترجمة حرفية كلمة-بكلمة.
- **استُخرِج كل نص مُثبَّت (hardcoded) في كل صفحة** (نصوص JSX،
  aria-label، placeholder، رسائل الأخطاء) إلى `t()` - لا نص عربي مُثبَّت
  متبقٍ في أي من الصفحات الـ21.
- **أصناف Tailwind الاتجاهية حُوِّلت لصيغة منطقية** (`text-right`→
  `text-start`، `mr-*`→`ms-*`، `pr-*`→`pe-*`، `border-r-*`→`border-s-*`،
  `right-*`→`start-*`، ...إلخ - نفس القاعدة بالعكس لليسار) في كل صفحة
  تحتوي هذه الأصناف، لأن التطبيق كان RTL فقط أصلًا فكانت "يمين/يسار"
  تُعبِّر فعليًا عن "بداية/نهاية" القراءة - مُتحقَّق بصريًا: التبديل
  للإنجليزية يقلب الشريط الجانبي لليسار والمحتوى بالكامل LTR بشكل صحيح
  دون كسر أي محاذاة.
- **تنسيق التاريخ/الوقت أصبح واعيًا باللغة** (`src/utils/formatDate.ts`):
  استُبدِلت كل استدعاءات `toLocaleString('ar-SA')`/`toLocaleDateString('ar-SA')`
  المُثبَّتة (13 موضعًا عبر 7 ملفات) بدالتين `formatDate`/`formatDateTime`
  تقرآن اللغة الحالية - العربية تبقى بالضبط كما كانت (`ar-SA`)،
  والإنجليزية تستخدم `en-GB` بدل البقاء عالقة بتنسيق عربي.

### عيب حقيقي اكتُشف وأُصلِح: جمع اللغة العربية في i18next
القاعدة الأولية `_one`/`_other` (المُطابِقة لتبسيط الكود الأصلي
`count === 1 ? ... : ...`) **غير كافية للعربية**: فئات الجمع الحقيقية
لـ`Intl.PluralRules('ar')` هي `zero/one/two/few/many/other` - فمثلًا
`count = 10` يُصنَّف `"few"`, وبما أن هذا المفتاح غير موجود يعيد i18next
**النص الخام غير المُترجَم** بدل أي fallback. اكتُشف عبر اختبار Vitest
حتمي (`SubscriptionPage`, `trialDaysRemaining: 10`) واختبار برمجي مباشر
لسلوك `Intl.PluralRules`. أُصلِح بإضافة كل الفئات الست
(`_zero/_one/_two/_few/_many/_other`) لكل مفتاح جمع عربي متأثر
(`subscription.json` و`nav.json`)، كلها بنفس نص الجمع الأصلي عدا `_one`.
الإنجليزية تحتاج `_one`/`_other` فقط (فئتا CLDR الإنجليزية) - عدم تطابق
المفاتيح بين ar/en لهذين المفتاحين فقط **متعمَّد وصحيح**، وليس خطأ.

### الاختبارات
Frontend: **53/53** Vitest (بلا أي تعديل على ملفات الاختبار نفسها).
Playwright: **4/4**. Lint/Typecheck/Build: نظيفة عبر كل الملفات الـ21
المُعدَّلة. تحقّق بصري كامل عبر Playwright يدويًا: تبديل اللغة من صفحة
الدخول، التنقّل بين الصفحات مع بقاء الإنجليزية، RTL↔LTR ينعكس بشكل صحيح
(الشريط الجانبي، المحاذاة)، بيانات العمل الفعلية (كأسماء الحسابات
المحاسبية المُنشأة فعليًا) تبقى كما أدخلها المستخدم بلا ترجمة قسرية -
تُرجِمت واجهة النظام فقط، لا محتوى المستخدم.

### لم يتغيّر
منطق العمل، مسارات API، صلاحيات RBAC، بنية أي مكوّن أو نوع أي prop
(باستثناء نقل خرائط تسمية ثابتة من مستوى الوحدة module-level إلى داخل
المكوّن حيث يمكن استدعاء `t()`، بلا تغيير سلوكي).

## [Frontend Typography: خط Tajawal مُستضاف ذاتيًا] - 2026-08-19

**ليست Milestone جديدة — متابعة مباشرة لـ"Frontend Design Refresh" أدناه،
بناءً على طلب المستخدم بخط أوضح وأكثر تميّزًا.** بلا تغيير في منطق العمل.

استُضيف خط **Tajawal** (نفس الخط المُجرَّب سابقًا عبر Google Fonts ثم
أُزيل) **محليًا هذه المرة**: مِلَفّات `.woff2` (عربي + لاتيني × أوزان
400/500/700/800، 8 ملفات، ~77KB إجمالاً) نزّلت مرّة واحدة من خادم Google
الثابت وأُدرِجت في `frontend/src/assets/fonts/` (مع `LICENSE.txt` -
SIL OFL 1.1) بدل تحميلها من الشبكة عند التشغيل. `tailwind.config.js`
يضع `Tajawal` أولاً في `fontFamily.sans` مع نفس حزمة خطوط النظام
كاحتياط. **الفرق الجوهري عن المحاولة السابقة**: لا طلب شبكي خارجي مطلقًا
الآن (الملفات جزء من حزمة البناء نفسها) - فلا خطر تعليق تنقّل متكرر، ولا
اعتماد على توفّر Google وقت التشغيل. مُتحقَّق منه: Playwright 4/4 (سريعة،
لا تعليق)، `npm run build` يُنتج الملفات كأصول ذات hash طبيعية.

### الاختبارات
Frontend: 53/53 Vitest، 4/4 Playwright. Lint/Typecheck/Build: نظيفة.

## [Frontend Design Refresh] - 2026-08-19

**ليست Milestone جديدة — تصميم بصري فقط، بلا تغيير في منطق العمل.** إعادة
تصميم بصرية للواجهة الأمامية (React/Tailwind، Milestones 1-10 القائمة)
لتكون أكثر حداثة ووضوحًا، بتطبيق مبادئ من مجموعة skills (better-ui،
better-typography، better-colors، better-accessibility، better-layout).
لم يتغيّر أي منطق عمل، مسار API، أو صلاحية RBAC.

### التغييرات
- **نظام الألوان** (`tailwind.config.js`): إعادة بناء لوني `brand` (نفس
  عائلة اللون الأخضر المُزرقّ الأصلية) و`accent` كسلالم كاملة متدرجة
  بانتظام بدلاً من 3 درجات متفرقة. الدرجة `600` مُتحقَّق منها صراحةً
  لتحقيق تباين WCAG AA (≥4.5:1) كنص/خلفية-نص-أبيض؛ الدرجة `500` لعناصر
  الحدود/الزخرفة غير النصية (≥3:1). أُصلِح استخدام وحيد سابق
  (`SubscriptionPage.tsx` badge "الخطة الحالية") كان سيفشل بصريًا (تباين
  منخفض) مع السلّم الجديد.
- **الطباعة**: حزمة خط نظام/منصة (`ui-sans-serif, system-ui,
  -apple-system, "Segoe UI", "Noto Sans Arabic", ...`) بدل الخط السابق —
  **جُرِّب خط ويب خارجي (Google Fonts) ثم أُزيل عمدًا**: تسبّب في تعليق
  فعلي (`page.goto` timeout) عبر Playwright بسبب إعادة الجلب على كل
  تنقّل صفحة كامل ضمن بروكسي هذه البيئة — خطر موثوق على الموثوقية في أي
  بيئة إنتاجية تخلف شبكة مقيّدة أيضًا (بروكسي شركات، حاجب إعلانات،
  انقطاع Google)، وليس فقط عيب اختبار. القرار النهائي: حزمة خط نظام لا
  تعتمد على أي طلب شبكي خارجي.
- **المكوّنات المشتركة** (`components/ui.tsx`): إعادة تصميم Button (ظل
  ناعم، انضغاطة عند النقر `scale(0.96)`)، Input/Select (حواف أنعم، ظل
  خفيف)، Card (زوايا أوسع، ظل ارتفاع بدل حدّ صلب فقط)، PageHeader (تسلسل
  عناوين أوضح)، ErrorBanner (أيقونة + `role="alert"`)، Modal، Pagination،
  و`Badge` جديد لشارات الحالة. هذه المكوّنات تُستخدَم عبر كل الصفحات الـ18
  تقريبًا فتنعكس التغييرات تلقائيًا.
- **القالب العام** (`components/Layout.tsx`): الشريط الجانبي بأيقونات
  SVG لكل عنصر تنقّل (بلا مكتبة خارجية جديدة)، حالة تنشيط أوضح، بطاقة
  مستخدم بصورة رمزية بالحرف الأول، سحب موبايل مع طبقة ضبابية خلفية —
  نفس روابط/بنية التنقّل الأصلية دون تغيير.
- **صفحات الدخول** (`LoginPage.tsx`, `RegisterPage.tsx`): خلفية متدرجة
  بلون العلامة، شعار "ق" مصغّر، بطاقة مركزية مُحسَّنة — بلا تغيير في أي
  تسمية حقل أو نص زر يعتمد عليه أي اختبار.
- **إصلاح خلل حقيقي اكتُشف أثناء التصميم**: `Modal` في `ui.tsx` كانت
  `useEffect` الخاصة بالتركيز التلقائي تعتمد على `onClose` ضمن قائمة
  الاعتماديات — بما أن `onClose` غالبًا دالة سطرية جديدة في كل تصيير،
  كانت الـeffect تُعاد عند أي إعادة تصيير (كل ضغطة مفتاح داخل نموذج
  ضمن Modal)، فتسرق التركيز (focus) رجوعًا إلى زر الإغلاق أثناء الكتابة.
  اكتُشف عبر فشل حتمي (غير متقطّع) في 4 اختبارات Vitest كانت تكتب مبلغًا
  متعدد الأرقام في نموذج داخل Modal ("PurchasesPage" وغيرها) - أُصلِح
  بفصل تأثير التركيز الأولي عن مستمع مفتاح Escape (الأخير يستخدم `ref`
  لآخر `onClose` بدل الاعتماد المباشر)، مع اختبار الانحدار الكامل
  للتأكد من عدم تكرّره.

### الاختبارات
Frontend: **53/53** Vitest (بما فيها الأربعة التي كشفت خلل Modal).
Playwright: **4/4** (بعد إزالة Google Fonts؛ كانت 4/4 تفشل بمهلة تنقّل
قبل الإصلاح). Lint/Typecheck/Build: نظيفة. لا تغيير في اختبارات
الـbackend (لم يُلمَس أي كود خلفي).

### لم يتغيّر
منطق العمل، مسارات API، صلاحيات RBAC، نص أي حقل/زر يعتمد عليه اختبار،
بنية التنقّل (نفس الروابط/الترتيب). صفحات المحتوى الداخلية (المنتجات،
المخزون، المشتريات، المحاسبة...) لم تُعدَّل يدويًا صفحة بصفحة - ترث
المظهر الجديد تلقائيًا عبر المكوّنات المشتركة وتوكنات Tailwind فقط.
...
لا كود عمل جديد (لم يُكتشَف عيب يستوجب إصلاحًا)، توثيق فقط. راجع
`docs/PROJECT_STATUS.md` §"تحقق Go-Live الإنتاجي" و`docs/DEPLOYMENT.md`
§"تحقق Go-Live فعلي" للتفصيل الكامل.

نُفِّذ تدفق نشر إنتاجي حقيقي محليًا (بلا Docker — daemon غير متاح، مُتحقَّق
منه صراحةً هذه الجلسة أيضًا): قاعدة بيانات جديدة تمامًا + `prisma migrate
deploy` من الصفر + سكربتات `manual-sql` الثلاثة + بذر + بدء `node
dist/main` بـ`NODE_ENV=production` حقيقي (رفض حي لسر placeholder، قبول حي
لأسرار قوية) + بناء واجهة أمامية إنتاجي حقيقي + مجموعة Playwright الكاملة
(4/4) ضد هذه الحزمة الحية + تحقق `curl` كامل لمصادقة تكامل قيّدها الخارجية
(401 لسر خاطئ، 201 لسر صحيح) + تحقق حي لـCORS Allowlist وHelmet Headers +
فحص مباشر لسجلات الخادم (بلا أي سر). قاعدة البيانات المؤقتة حُذفت بعد
التحقق؛ dev/test لم تُلمَسا.

### الاختبارات
Backend: **242/242** e2e (بلا تغيير)، **9/9** unit. Frontend: **53/53**
Vitest. Playwright: **4/4** — نُفِّذت مرتين في هذه الجلسة: مرة ضد الحزمة
الإنتاجية الحية الموصوفة أعلاه.

### متطلبات خارجية متبقية (بلا تغيير عن Milestone 10)
دومين/HTTPS حقيقيان، Reverse proxy، Docker daemon، استضافة/نشر
Demo-Staging فعلي، نسخ احتياطي مُجدوَل آليًا — كلها تتطلب إجراء المُشغِّل،
موثَّقة بالتفصيل في `docs/PROJECT_STATUS.md`.
